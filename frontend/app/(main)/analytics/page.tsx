...
const DEEP_DIVES: AnalysisArticle[] = [
  ...
  },
  {
    ...
    excerpt: "Expected Goals (xG) is reshaping how we understand football. We break down the metric and what it really tells...",
    author: 'Priya Sharma',
    readTime: '7 min',
  },
  {
    ...
    excerpt: 'One full season of Jude Bellingham at Madrid — and the numbers reveal a player operating at a level rarely seen...',
    author: 'Rafael Costa',
    readTime: '8 min',
  },
]
      ...
      <div className="bg-[#111111] dark:bg-[#111111] border-b border-border py-6">
        <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-black text-white uppercase tracking-wide">Analytics</h1>
          </div>
          <p className="text-[13px] text-[#888]">xG, ratings, tactical breakdowns and the data behind every match</p>
        </div>
      </div>
          ...
          {STAT_CARDS.map(({ icon: Icon, value, label, delta, positive }) => (
            <div key={label} className="rounded-xl border border-border bg-card p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-[12px] font-bold ${positive ? 'text-green-500' : 'text-red-500'}`}>
                  {delta}
                </span>
              </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-3 shrink-0 ml-2 text-[12px]">
                        <span className="text-muted-foreground">
                          xG <span className="font-bold text-foreground">{player.xg}</span>
                        </span>
              ...
              {POWER_RANKINGS.map((team) => (
                <div key={team.rank} className="flex items-center justify-between py-3.5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`text-[12px] font-bold w-10 text-right tabular-nums ${
                        team.delta >= 0 ? 'text-green-500' : 'text-red-500'
                      }`}
                    >
            ...
            {DEEP_DIVES.map((article) => (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <h3 className="text-[15px] font-black leading-snug mt-1.5 mb-2 group-hover:text-primary transition-colors">
                    {article.title}
                  </h3>
